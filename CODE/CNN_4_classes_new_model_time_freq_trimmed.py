# 10-fold cross-validation with the new model on the dataset with 4 classes, first 10 timebins and
# first 13 frequency bins trimmed
# PVHA, PVLA, NVHA, NVLA,
...
# average over trimmed time and also select frequencies from lower beta
X = np.mean(X[:, 13:, 10:, :], axis=2)
...
with plt.style.context("default"):
    ...
    plt.tight_layout()
    plt.savefig('FIGURES/trainTestDist_cnn_10fold_4_classes_new_time_freq_trimmed.pdf')
...
accTrain_fn = ('REPORTS/accuracyTrain_4_new_time_freq_trimmed.p')
with open(accTrain_fn, 'wb') as fp:
    pickle.dump(accuracyTrain, fp)
accVal_fn = ('REPORTS/accuracyVal_4_new_time_freq_trimmed.p')
...
# Save history
accTotal_4_fn = ('REPORTS/accuracyTotal_4_new_time_freq_trimmed.p')
with open(accTotal_4_fn, 'wb') as fp:
    pickle.dump(acc_mean, fp)

reports_4_fn = ('REPORTS/reportsCNN_4_new_time_freq_trimmed.p')
with open(reports_4_fn, 'wb') as fp:
    pickle.dump(reports, fp)

confusionMatrices_4_fn = ('REPORTS/confusionMatricesCNN_4_new_time_freq_trimmed.p')
...
with plt.style.context("default"):
    ...
    plt.grid
    plt.savefig('FIGURES/accTrainVal_10foldCV_4_classes_new_time_freq_trimmed.pdf')
...
plt.tight_layout()
plt.savefig('FIGURES/normCM_cnn_10fold_4_classes_new_time_freq_trimmed.pdf')
...
with plt.style.context("default"):
    ...
    plt.tight_layout()
    plt.savefig('FIGURES/accuracyTrainVal_cnn_10fold_4_classes_new_time_freq_trimmed.pdf')

# save model
h5_fn = ('MODELS/CNN_4_classes_new_model_time_freq_trimmed.h5')
model.save(h5_fn)
